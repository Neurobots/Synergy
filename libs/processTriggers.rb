  ...
  returnset = 0
	...
	def trigger_speak( trigger, name )
		if trigger['use_saying_switch'].to_i == 1
  		self.client.room.say(@sayings.sample)
		else
  		if trigger['use_name_switch'].to_i == 1
    		self.client.room.say(trigger['pre_name_response']+name+trigger['post_name_response']) if (trigger['pre_name_response'] != "")||(trigger['post_name_response'] != "")
  		else
    		self.client.room.say(trigger['pre_name_response']+trigger['post_name_response']) if (trigger['pre_name_response'] != "")||(trigger['post_name_response'] != "")
  		end
		end
	end
      	...
      	if validate_security(message.sender.id, trigger['access_level'].to_i)
        	case trigger['action']
              ...
              if ( from.abs < @queue.count ) && ( to.abs < @queue.count )
              	self.client.room.say("Moving \##{from} to \##{to}")
            	...
            	if @botData['queue']
              	if @queue.count != 0
                	@queue.each_with_index do |user, i|
                  	self.client.room.say("Queue Slot \##{i} #{user.name}")
                  end
                else
            ...
            when "*queue_add"
            	if @botData['queue']
              	if !@tabledjs.include?(message.sender)
                	if @tabledjs.count == 5
                      ...
                      self.client.room.say("Ok #{message.sender.name}, you are #{@queue.count} in the queue")
                    else
                    	self.client.room.say("I'm sorry #{message.sender.name}, but you are already in the queue")
                    end
                  else
            	...
            	if @botData['queue']
                  ...
                  self.client.room.say("Ok #{message.sender.name}, I'm removing you from the queue")
                else
                	self.client.room.say("I'm sorry #{message.sender.name}, but you aren't in the queue right now")
                end
              else
              	self.client.room.say("The queue is not enabled right now.  Fastest finger right now.")
              end
            ...
            when "*kick"
              ...
              if @botData['ownerid'].match(/#{userid}/)
              	...
              	URI.parse("http://www.neurobots.net/websockets/blacklistpush.php?magic_key=#{self.magickey}&target=#{userid}&reason=#{message.sender.id}").read
            ...
            when "*removedj"
              ...
              if (trigger['post_command_fail'] == "")
              	self.client.room.say(trigger['pre_command_fail'])
              else
              	self.client.room.say("#{trigger['pre_command_fail']}#{@errorcounts['fan']}#{trigger['post_command_fail']}")
                @errorcounts['fan'] += 1
              end
            end
          end
