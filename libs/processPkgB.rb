module Processpkgb
    ...
    if ( @botData['pkg_b_data']['wikipedia_e'] == "1" ) and ( message.content.match(/^#{Regexp.escape(@botData['pkg_b_data']['wikipedia_t'])} /))
  	...
  	if ( @botData['pkg_b_data']['user_lookup_e'] == "1" ) and ( message.content.match(/^#{Regexp.escape(@botData['pkg_b_data']['user_lookup_t'])} /))
    	...
    	self.db.query("select * from bot_ustats_#{self.magickey} where LOWER(name) = LOWER('#{self.db.escape_string(search)}')  LIMIT 1").each do |row|
      	case  loc
    			when 0
      			self.client.room.say("#{row[1]}:")
      			self.client.room.say("#{row[2]}")
      			self.client.room.say("[#{row[4]}:thumbsup:][#{row[5]}:thumbsdown:][#{row[6]}<3][#{row[3]}:dvd:]")
