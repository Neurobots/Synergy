module Songended
    ...
    self.client.on :song_ended do |song|
  		...
  		EventMachine::Timer.new(1) do
    		Turntabler.run { self.client.user.remove_as_dj if (self.client.user.dj?)and(self.client.room.djs.count == 1)and(!@botData['alonedj'])and(@botData['autodj'])and(@last_dj == self.client.user) }
			end
			
			#puts PP.pp(song, "").yellow
			@autobop_count = 0
      if song.played_by != self.client.user
                self.db.query("insert into bot_sstats_#{self.magickey} set songid='#{digest(song.title, song.artist)}', title='#{self.db.escape_string(song.title)}', artist='#{self.db.escape_string(song.artist)}', first_user_played='#{song.played_by.id}' on duplicate key update times_played=times_played")
								self.db.query("update bot_sstats_#{self.magickey} set times_awesomed=times_awesomed+#{song.up_votes_count}, times_lamed=times_lamed+#{song.down_votes_count} where songid='#{digest(song.title, song.artist)}'")
                ...
                self.db.query("select times_awesomed, times_lamed, times_snagged, times_played from bot_sstats_#{self.magickey} where songid='#{digest(song.title, song.artist)}' limit 1").each do |lsong|
                ...
                song.votes.each do |vote|
                	...
                	self.db.query("update bot_ustats_#{self.magickey} set songs_awesomed=songs_awesomed+1 where userid='#{vote.user.id}'") if vote.direction == :up
                	self.db.query("update bot_ustats_#{self.magickey} set songs_lamed=songs_lamed+1 where userid='#{vote.user.id}'") if vote.direction == :down
        				end
        end
